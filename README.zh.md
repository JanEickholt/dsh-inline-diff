...
每行两侧都有 1 基行号，位于柔和的行号栏中。行号来自 harness 本身：编辑落定时，插件会把每个 served 的 hunk 锚定到真实位置；没有锚点的 hunk 则通过受限的工作区读取路由读取文件，逐字定位变更块。无法定位的 hunk（块出现多次、文件已漂移、宿主半边缺席）会退回到从 hunk 首行起算的窗口相对编号，行号栏因此始终有数字——但那可能不是文件里的真实行号。想更清爽？在 **设置 → 插件 → 行内 Diff → 行号** 下选择*关*，行号栏完全消失。与其他选择一样，保存且重启后生效。
