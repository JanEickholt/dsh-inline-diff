...
`;
		...
		function buildRows(ops, oldLines, newLines) {
			...
			while (opIndex < ops.length) {
				...
				for (let pairIndex = pairedCount; pairIndex < removedRun.length; pairIndex++) {
					rows.push({ kind: "removed", leftText: oldLines[removedRun[pairIndex].oldIndex] });
				}
				for (let pairIndex = pairedCount; pairIndex < addedRun.length; pairIndex++) {
					rows.push({ kind: "added", rightText: newLines[addedRun[pairIndex].newIndex] });
				}
		...
		function pushCellPair(leftCells, rightCells, rowIndex, row, wordDiff, lang, syntaxOn) {
			const leftText = row.leftText != null ? row.leftText : "";
			const rightText = row.rightText != null ? row.rightText : "";
			let left;
			let right;
			if (row.kind === "removed") {
				left = codeCell("did-code did-delbg", leftText, lang, syntaxOn, null, "");
				right = cell("did-code did-void", "");
			} else if (row.kind === "added") {
				left = cell("did-code did-void", "");
				right = codeCell("did-code did-insbg", rightText, lang, syntaxOn, null, "");
				...
				if (!wordDiff) {
					...
					left = codeCell("did-code did-delbg", leftText, lang, syntaxOn, segments.left, "did-delword");
					right = codeCell("did-code did-insbg", rightText, lang, syntaxOn, segments.right, "did-insword");
				}
			} else {
				left = codeCell("did-code", leftText, lang, syntaxOn, null, "");
				right = codeCell("did-code", rightText, lang, syntaxOn, null, "");
			}
		...
		function InlineDiffRow(props) {
			...
			// data-ri cells' heights so counterpart rows stay level.
			react.useEffect(() => {
			...
			for (const hunk of hunks) {
				...
				for (const row of rows) {
					...
					pushCellPair(leftCells, rightCells, rowCounter++, row, words, lang, syntaxOn);
					renderedRows++;
				}
