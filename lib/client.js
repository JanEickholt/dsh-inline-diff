window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region stylesheet
		const CSS = `
.did-root{font-size:12px;line-height:19px;margin:4px 0 2px;border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.25));border-radius:6px;overflow:hidden;background:var(--dsw-alias-bg-layer-2,#2c2c2e)}
...
.did-stats{margin-left:auto;color:var(--dsw-alias-label-tertiary,#6f6f6f);font-size:11px;white-space:nowrap}
...
.did-filehead{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:3px 8px;background:rgba(255,255,255,.04)}
...
.did-no{text-align:right;padding:0 6px;color:var(--dsw-alias-label-tertiary,#6f6f6f);opacity:.7;user-select:none;font-size:10.5px;font-family:var(--ds-font-family-code,ui-monospace,SFMono-Regular,Menlo,Consolas,monospace)}
.did-code{padding:0 8px;color:var(--dsw-alias-label-primary,#d4d4d4);white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;min-width:0;font-family:var(--ds-font-family-code,ui-monospace,SFMono-Regular,Menlo,Consolas,monospace)}
.did-delbg{background:rgba(248,81,73,.08)}
.did-insbg{background:rgba(63,185,80,.08)}
.did-no.did-delbg{background:rgba(248,81,73,.18)}
.did-no.did-insbg{background:rgba(63,185,80,.18)}
.did-delword{background:rgba(248,81,73,.32);border-radius:2px}
.did-insword{background:rgba(63,185,80,.30);border-radius:2px}
.did-void{background:rgba(128,128,128,.04)}
.did-more{padding:4px 8px;color:var(--dsw-alias-label-tertiary,#6f6f6f);font-size:11px;border-top:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.2))}
`;
		//#endregion
		...
		/**
		 * Strip the leading whitespace shared by every non-empty line of both
		 * sides (computed jointly, empty lines ignored). Relative indentation
		 * differences survive: a line indented deeper than the common prefix
		 * keeps its extra indent on either side.
		 */
		function dedentPair(a, b) {
			let prefix = null;
			for (const lines of [a, b]) {
				for (const line of lines) {
					...
					let i = 0;
					const min = Math.min(prefix.length, ws.length);
					while (i < min && prefix[i] === ws[i]) i++;
					prefix = prefix.slice(0, i);
					if (prefix === "") return [a, b];
				}
			}
			if (!prefix) return [a, b];
			const strip = (lines) => lines.map((l) => (l.startsWith(prefix) ? l.slice(prefix.length) : l.trim() ? l : ""));
		...
		/** Token-level LCS ops (same shape as lcsOps), or null past the cap. */
		function tokenOps(a, b) {
			const n = a.length, m = b.length;
			if (n > 400 || m > 400) return null;
			const w = m + 1;
			const dp = new Int32Array((n + 1) * w);
			for (let i = n - 1; i >= 0; i--) {
				const row = i * w, below = (i + 1) * w;
		...
		/**
		 * Split a changed line pair into per-side segments [{text, hl}]: changed
		 * token runs carry hl (rendered as stronger chips), unchanged runs keep
		 * the row tint.
		 */
		function wordSegments(oldLine, newLine) {
			...
			if (ops === null) {
				return { l: [{ text: oldLine, hl: true }], r: [{ text: newLine, hl: true }] };
			}
			const merge = (side) => {
				const segs = [];
				for (const op of ops) {
					const take = side === "l" ? (op.t !== "in") : (op.t !== "dl");
					if (!take) continue;
					const text = side === "l" ? a[op.ai] : b[op.bi];
					const hl = op.t !== "eq";
					const last = segs[segs.length - 1];
					if (last && last.hl === hl) last.text += text;
					else segs.push({ text, hl });
				}
				return segs;
			};
		...
		function segCell(cls, segs, hlCls) {
			const children = segs.map((s, i) => s.hl
				? react.createElement("span", { className: hlCls, key: i }, s.text)
				: s.text);
		...
		function pushRow(cells, row) {
			...
			if (row.kind === "del") {
				...
				const segs = wordSegments(ltext, rtext);
				cells.push(segCell("did-no did-delbg", [{ text: lno, hl: false }], ""), segCell("did-code did-delbg", segs.l, "did-delword"), segCell("did-no did-insbg", [{ text: rno, hl: false }], ""), segCell("did-code did-insbg", segs.r, "did-insword"));
			} else {
		...
		function InlineDiffRow(props) {
			...
			// Widest rendered line per side, in characters (the code font is
			// monospace, so px = chars × measured char width). Drives the
			// fitted card width: equal halves, centered in the conversation
			// column, capped at the container width.
			let maxL = 0, maxR = 0;
			if (diffs !== null) {
				for (const h of diffs) {
					const [a, b] = dedentPair(splitLines(h.oldText), splitLines(h.newText));
					for (const line of a) if (line.length > maxL) maxL = line.length;
					for (const line of b) if (line.length > maxR) maxR = line.length;
				}
			}

			// Fitted width + centering: measured at mount, when line widths
			// change, and on container/window resize; no-op without a
			// container.
			const rootRef = react.useRef(null);
			const [expandStyle, setSizeStyle] = react.useState(null);
			react.useEffect(() => {
				const el = rootRef.current;
				if (!el || typeof ResizeObserver === "undefined") return;
				const container = el.closest("[data-conversation-scroll]");
				if (!container) return;
				const INSET = 12, GUTTER = 76, CELL_PAD = 32, CHROME = 6, MIN = 480;
				let ctx = null;
				const measure = () => {
					const cRect = container.getBoundingClientRect();
					if (cRect.width < 60) return;
					// Cap at 3/4 of the scroll container width; long lines wrap.
					const maxPx = Math.floor(cRect.width * 3 / 4);
					const codeEl = el.querySelector(".did-code");
					if (!ctx) ctx = document.createElement("canvas").getContext("2d");
					if (codeEl) {
						const cs = getComputedStyle(codeEl);
						ctx.font = cs.fontSize + " " + cs.fontFamily;
					}
					const charW = ctx.measureText("0").width || 7.2;
					const sidePx = Math.max(maxL, maxR) * charW;
					let w = Math.ceil(sidePx * 2 + GUTTER + CELL_PAD + CHROME);
					w = Math.max(Math.min(MIN, maxPx), Math.min(w, maxPx));
					const applied = parseFloat(el.style.marginLeft) || 0;
					const staticLeft = el.getBoundingClientRect().left - applied;
					const marginLeft = Math.round(cRect.left + (cRect.width - w) / 2 - staticLeft);
					setSizeStyle((prev) =>
						prev && prev.width === w + "px" && prev.marginLeft === marginLeft + "px"
							? prev
							: { width: w + "px", marginLeft: marginLeft + "px" }
					);
				};
				measure();
			...
			if (diffs === null) {
				return react.createElement("div", { className: "did-root", ref: rootRef, style: expandStyle || undefined },
