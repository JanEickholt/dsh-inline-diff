...
.did-indent{color:var(--dsw-alias-label-tertiary,#6f6f6f);font-size:10.5px;margin-right:10px}
...
`;
		 ...
		 * keeps its extra indent on either side. Returns { a, b, indent } with
		 * indent = stripped character count (0 = nothing stripped).
		 */
		...
		function InlineDiffRow(props) {
			...
			// Fixed width: always 3/4 of the conversation scroll container,
			// centered — every diff card is the same size. Measured at mount
			// and on container/window resize; no-op without a container.
			...
			react.useEffect(() => {
				...
				const measure = () => {
					...
					const w = Math.floor(cRect.width * 3 / 4);
			...
			for (const h of diffs) {
				const d = dedentPair(splitLines(h.oldText), splitLines(h.newText));
				const a = d.a, b = d.b;
				let ops = lcsOps(a, b);
				if (ops === null) {
					ops = [];
				...
				children.push(react.createElement("div", { className: "did-file", key: h.path },
					react.createElement("div", { className: "did-filehead" },
						...
						react.createElement("span", { className: "did-stats" },
							d.indent > 0 ? react.createElement("span", {
								className: "did-indent",
								title: "common indentation stripped (" + d.indent + " chars)"
							}, "⇤ " + d.indent) : null,
							" ",
