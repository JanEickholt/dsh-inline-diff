...
.did-grid {
	...
	/* gutters inherit the code font so rows stay aligned when a theme rescales it */
...
`;
		...
		// oldBase/newBase carry the hunk's 0-based offset inside its file when
		// the host stamps one, so gutters show real line numbers and degrade
		// to plain 1..n otherwise.
		...
		// Validate untrusted wire diffs into well-formed hunks. The host may
		// stamp 1-based oldStart/newStart (the hunk's first line inside its
		// file); anything but a finite integer means unknown position.
		...
		// Result view wins once settled; new-file writes and still-running
		// calls only ever have the call-time view.
		...
		// Card width: CARD_WIDTH_RATIO of the conversation column, never below
		// the reply-text column (--dsh-chat-content-width) minus a gutter,
		// centered via margin so cards match. Null on tiny containers.
