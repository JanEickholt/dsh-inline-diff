		...
		function InlineDiffRow(props) {
			...
			if (diffs !== null) {
				for (const h of diffs) {
					for (const line of splitLines(h.oldText)) if (line.length > maxL) maxL = line.length;
					for (const line of splitLines(h.newText)) if (line.length > maxR) maxR = line.length;
				}
			}
