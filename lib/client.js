...
`;
		...
		function InlineDiffRow(props) {
			...
			// Full-bleed expansion: widen the card to the conversation scroll
			// container (the chat column is centered with a narrow max-width).
			// Measured at mount and on container/window resize; a no-op fallback
			// when no container can be found.
			...
			react.useEffect(() => {
				...
				const measure = () => {
					...
					const staticLeft = eRect.left - applied;
					const width = Math.floor(cRect.width - INSET * 2);
					const marginLeft = Math.round(cRect.left + INSET - staticLeft);
					setExpandStyle((prev) =>
						prev && prev.width === width + "px" && prev.marginLeft === marginLeft + "px"
							? prev
