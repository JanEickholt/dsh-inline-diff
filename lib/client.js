...
`;
		...
		// its true counterpart renders as a pure addition. Dice over token
		// multisets (same tokenizer as the word chips); weak matches stay
		...
		// Sørensen–Dice over token multisets: 1 = identical, 0 = disjoint.
		function lineSimilarity(oldTokens, newTokens) {
			let shared = 0;
			...
			return (2 * shared) / (oldTokens.size + newTokens.size);
		}
		...
		function pairRunLines(oldLines, removedRun, newLines, addedRun) {
			...
			for (let oldSide = 0; oldSide < removedRun.length; oldSide++) {
				for (let newSide = 0; newSide < addedRun.length; newSide++) {
					const similarity = lineSimilarity(oldTokens[oldSide], newTokens[newSide]);
					if (similarity >= PAIR_SIMILARITY_MIN) candidates.push({ oldSide, newSide, similarity });
				}
			}
			...
			for (const candidate of candidates) {
				...
				pairs.push([removedRun[candidate.oldSide].oldIndex, addedRun[candidate.newSide].newIndex]);
			}
		...
		function buildRows(ops, oldLines, newLines) {
			...
			while (opIndex < ops.length) {
				...
				for (const op of removedRun) {
					if (!usedOld.has(op.oldIndex)) {
						rows.push({ kind: "removed", leftText: oldLines[op.oldIndex], oldIndex: op.oldIndex });
					}
				}
				for (const op of addedRun) {
					if (!usedNew.has(op.newIndex)) {
