...
`;
		...
		function buildRows(ops, oldLines, newLines) {
			...
			while (opIndex < ops.length) {
				...
				// block. undefined (pure runs, run too big) falls back to the
				// per-pair word diff / no chips downstream.
		...
		function pushCellPair(leftCells, rightCells, rowIndex, row, wordDiff, lang, syntaxOn, oldBase, newBase, numbersOn) {
			...
			if (row.kind === "removed") {
				...
				if (!wordDiff) {
					...
					const segments = row.leftSegments !== undefined
						? { left: row.leftSegments, right: row.rightSegments }
						: wordSegments(leftText, rightText);
