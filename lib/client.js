window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
.did-root {
	/* host theme token first, fixed fallback second */
	...
	--did-filehead-bg: rgba(255, 255, 255, 0.04);
	--did-empty-bg: rgba(128, 128, 128, 0.04);

	/* add/remove palette: stat color, row fill, number-gutter fill, word chip */
	--did-add-stat: #3fb950;
	--did-add-row: rgba(63, 185, 80, 0.08);
	--did-add-gutter: rgba(63, 185, 80, 0.18);
	--did-add-word: rgba(63, 185, 80, 0.3);
	--did-del-stat: #f85149;
	--did-del-row: rgba(248, 81, 73, 0.08);
	--did-del-gutter: rgba(248, 81, 73, 0.18);
	--did-del-word: rgba(248, 81, 73, 0.3);
...
/* Settings → General row: "Diff highlighting" with a Words/Lines segmented control */
.did-setting {
	display: flex;
	align-items: center;
	justify-content: space-between;
...
`;
		...
		//#region highlight mode setting
		// Client-side view of the durable `inline-diff.highlight` preference
		// ("words" | "lines"). apply() keeps this synced from the bound settings
		// scope; diff cards and the settings row both subscribe here. The literal
		// strings must match lib/index.js — the module-loader bundle cannot import
		// across halves.
		...
		function InlineDiffRow(props) {
			...
			for (const hunk of hunks) {
				const stripped = stripSharedIndent(splitLines(hunk.oldText), splitLines(hunk.newText));
					...
					react.createElement("div", { className: "did-filehead" },
						...
						react.createElement("span", { className: "did-stats" },
							stripped.indentChars > 0 ? react.createElement("span", {
								...
								title: "common indentation stripped (" + stripped.indentChars + " chars)"
			...
			if (hiddenRows > 0) {
				children.push(react.createElement("div", { className: "did-more" }, "… " + hiddenRows + " more lines (truncated)"));
			}
			...
			return react.createElement("div", { className: "did-root", ref: rootRef, style: cardStyle || undefined },
				react.createElement("div", { className: "did-head" },
					react.createElement("span", { className: "did-tool" }, label),
					react.createElement("span", { className: "did-stats" },
						hunks.length > 1 ? hunks.length + " files · " : "",
		...
		//#region settings row
		// "Diff highlighting" row in Settings → General: Words (token chips on
		// paired rows) vs Lines (row tint only). Reads through the shared
		// wordsMode subscription; writes go through the injected setHighlight,
		// which echoes optimistically and lets the scope subscription confirm.
		function segmentButton(mode, label, active, choose) {
		...
		function DiffHighlightRow(props) {
			const setHighlight = props && props.setHighlight;
			const [words, setWords] = react.useState(getWordsMode());
			react.useEffect(() => onWordsMode(setWords), []);
			const choose = typeof setHighlight === "function"
				? setHighlight
				: () => {};
			return react.createElement("div", { className: "did-setting" },
				react.createElement("span", { className: "did-setting-title" }, "Diff highlighting"),
				react.createElement("div", { className: "did-seg", role: "group", "aria-label": "Diff highlighting" },
					segmentButton(HIGHLIGHT_WORDS, "Words", words, choose),
					segmentButton(HIGHLIGHT_LINES, "Lines only", !words, choose)
				)
			);
		}
		//#endregion
		...
		//#region plugin body
		// Install the stylesheet, adopt the durable highlight preference, then
		// shadow the stock edit/write keyed views and register the General
		// settings row. Lowest priority renders, so unloading this plugin
		// restores stock rows.
		function apply(ctx) {
			ctx.effect(() => {
			...
			ctx.slots.inject("settings.general.item", () => ctx.slots.register({
				...
				order: 20,
				inject: () => ({ setHighlight: writeHighlight })
			}, DiffHighlightRow));
		}
