...
/* Real 1-based file line numbers; blank when no trustworthy base exists. */
.did-num {
	position: absolute;
	left: 8px;
	width: 34px;
...
`;
		...
		// read through the host half's fenced read route. Every failure mode
		// yields null and the gutter cell stays blank — the card never shows a
		// guessed number.
		...
		function pushCellPair(leftCells, rightCells, rowIndex, row, wordDiff, lang, syntaxOn, oldBase, newBase) {
			...
			const leftNum = oldBase !== null && row.oldIndex !== undefined ? oldBase + row.oldIndex : null;
			const rightNum = newBase !== null && row.newIndex !== undefined ? newBase + row.newIndex : null;
			let left;
			let right;
			if (row.kind === "removed") {
		...
		function InlineDiffRow(props) {
			...
			react.useEffect(() => {
				...
				if (hunks === null || failed || cwd === undefined) return;
				const wanted = [];
				...
				void (async () => {
					...
					if (alive) setLocated({ key: hunkKey, values: next });
				})();
				return () => { alive = false; };
			}, [hunkKey, props ? props.cwd : undefined, failed]);
			...
			for (let hunkIndex = 0; hunkIndex < hunks.length; hunkIndex++) {
				...
				// Stamp bases win; located bases fill the gap; unknown stays null
				// and that side's gutter cells render empty.
