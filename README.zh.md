...
每行两侧都有真实的 1 基文件行号，位于柔和的行号栏中。行号来自 harness 本身：编辑落定时，插件会把每个 served 的 hunk 锚定到真实位置；没有锚点的 hunk 则通过受限的工作区读取路由读取文件，逐字定位变更块——出现多次的块、或读取后已变化的文件宁可拒绝猜测、不显示行号，也不会显示错误的行号。
