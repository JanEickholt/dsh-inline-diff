...
.did-root, .did-card {
	/* host theme tokens first, fixed fallback second. Surfaces ride the native
	   code-block token family (--dsw-alias-markdown-code-block-*), the same
	   tokens .md-code-block renders with, so cards match the code surface and
	   its banner under any theme or stylevault preset. Fallbacks are the
	   native dark statics; they only fire before theme CSS has injected. */
	...
	--did-add-gutter: color-mix(in srgb, var(--dsw-alias-state-success-primary, #22c55e) 18%, transparent);
	...
	--did-del-gutter: color-mix(in srgb, var(--dsw-alias-state-error-primary, #f25a5a) 18%, transparent);
	--did-del-word: color-mix(in srgb, var(--dsw-alias-state-error-primary, #f25a5a) 30%, transparent);
}

/* dsh-stylevault repaints native banners through element rules — brand 12%
   over the code surface plus a label-tertiary separator — and leaves the
   banner *alias* a flat elevation gray, so consuming the alias alone would
   diverge from what native headers actually show. The head watcher toggles
   .did-sv-themed on <html> while stylevault's paint layers are live, so diff
   cards copy that rendered look only when stylevault is styling the GUI;
   without it, the native alias above stands. The Plugins-page card is
   excluded: it rides the page's bg-layer ladder below, which stylevault
   re-themes through the same alias tokens. */
...
.did-root {
	...
	padding: 0;
	margin: 4px 0 2px;
	/* native code blocks carry no border, outline or shadow — the banner bar
	   anchors the card, so the diff card ships borderless in every mode */
	border-radius: 12px;
	overflow: hidden;
...
.did-grid {
	...
	/* code-block font shorthand (13px/22px + theme family); gutters inherit it
	   so rows stay aligned when a theme rescales code */
...
/* Plugins-page card: "Inline diff" accordion around the highlight control.
   Remap the surface tokens to the page's bg-layer ladder — the ladder every
   native settings card on this page renders with — so the card matches its
   siblings under the stock theme and under any stylevault preset (stylevault
   re-themes these same alias tokens). The code-block family above stays
   reserved for the diff cards in the conversation. */
.did-card {
...
`;
		...
		// up as "modified" rows; leftovers stay one-sided. bases carry the
		// 0-based line offset of the hunk inside its file (0 when unstamped),
		// so gutters show real file positions whenever the host half can stamp
		// them and degrades to plain 1..n counting otherwise.
		function buildRows(ops, oldLines, newLines, oldBase, newBase) {
			...
			while (opIndex < ops.length) {
				...
				if (op.type === "equal") {
					rows.push({
						kind: "context",
						leftNumber: oldBase + op.oldIndex + 1,
						rightNumber: newBase + op.newIndex + 1,
				...
				for (let pairIndex = 0; pairIndex < pairedCount; pairIndex++) {
					rows.push({
						kind: "modified",
						leftNumber: oldBase + removedRun[pairIndex].oldIndex + 1,
						rightNumber: newBase + addedRun[pairIndex].newIndex + 1,
				...
				for (let pairIndex = pairedCount; pairIndex < removedRun.length; pairIndex++) {
					rows.push({ kind: "removed", leftNumber: oldBase + removedRun[pairIndex].oldIndex + 1, leftText: oldLines[removedRun[pairIndex].oldIndex] });
				}
				for (let pairIndex = pairedCount; pairIndex < addedRun.length; pairIndex++) {
					rows.push({ kind: "added", rightNumber: newBase + addedRun[pairIndex].newIndex + 1, rightText: newLines[addedRun[pairIndex].newIndex] });
				}
		...
		// newText } hunks. The host half may stamp 1-based `oldStart`/
		// `newStart` onto each hunk (the hunk's first line inside its file);
		// finite integers ride along, anything else means unknown position.
		function optionalAnchor(value) {
			return typeof value === "number" && Number.isFinite(value) && value >= 1 && value % 1 === 0
				? value
				: null;
		}
			...
			for (const candidate of rawDiffs) {
				if (candidate && typeof candidate === "object"
					...
					hunks.push({
						...
						oldStart: candidate.oldText == null ? null : optionalAnchor(candidate.oldStart),
						newStart: optionalAnchor(candidate.newStart),
					});
				}
			}
		...
		function InlineDiffRow(props) {
			...
			for (const hunk of hunks) {
				...
				const { rows, added, removed } = buildRows(ops, stripped.oldLines, stripped.newLines,
					(hunk.oldStart ?? 1) - 1, (hunk.newStart ?? 1) - 1);
				totalAdded += added;
		...
		function apply(ctx) {
			...
			// Track whether dsh-stylevault is painting the GUI: its <style> layers
			// in <head> are the marker (empty when disabled). Toggles .did-sv-themed
			// on <html> so the stylesheet can switch the card chrome between the
			// native token look and stylevault's rendered banner recipe, live.
			ctx.effect(() => {
